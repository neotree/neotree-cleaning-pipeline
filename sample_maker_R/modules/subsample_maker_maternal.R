...
################################################################################
# Neotree Sample Maker -- Subsample Maker: Maternal Files  (module)
# FILE:    modules/subsample_maker_maternal.R
# PURPOSE: Create analysis-ready subsamples from cleaned maternal outcome files.
#          Supports combined_maternity_outcomes (Malawi) and maternal_outcomes
#          (Zimbabwe).  These are standalone files -- no joining or matching.
#
# DATE COLUMN
#   Filters on "dateadmission", which is present in both Malawi and Zimbabwe
#   maternal files.
#
# EXPORTED FUNCTIONS
#   run_subsample_maker_maternal(df, cfg)
#       Applies date/facility filtering, optional exclusion filters, and optional
#       column selection to a maternal outcomes data frame.
#       Returns a list:  subsample, label, report_text.
#
#   write_subsample_outputs_maternal(result, cfg, prefix, out_dir)
...
# ==============================================================================
.mat_build_filter_info <- function(cfg) {
...
# ==============================================================================
.mat_apply_filter <- function(df, cfg) {
...
# ==============================================================================
.mat_apply_exclusion_filters <- function(df, filters) {
...
# ==============================================================================
.mat_select_columns <- function(df, sub_variables) {
  if (is.null(sub_variables) || length(sub_variables) == 0) return(df)
  mandatory <- c("uid", "facility", "uniquekey", "dateadmission")
...
# ==============================================================================
.mat_fac_breakdown <- function(df, label) {
  if (nrow(df) == 0) {
...
# ==============================================================================
.mat_col_dist <- function(df, col_name, label) {
...
# ==============================================================================
.mat_build_report <- function(df_in, df_out, n_unparsed, cfg, filter_info) {
  ...
  lines <- c(
    sep_thick,
    "  Neotree Sample Maker -- Maternal Subsample Report",
    sep_thick,
    "",
  ...
  lines <- c(lines,
    ...
    "  Date column : dateadmission",
    sprintf("  Unparseable : %d rows with missing or unparseable dateadmission",
            n_unparsed)
  )
  ...
  lines <- c(lines, "", .mat_fac_breakdown(df_in, "Facilities in input"), "")
  ...
  lines <- c(lines, "", .mat_fac_breakdown(df_out, "Facilities in subsample"), "")

  # ---- Outcome distributions ----
  for (col in c("matoutcome", "neotreeoutcome", "modedelivery", "typebirth")) {
    dist_txt <- .mat_col_dist(df_out, col, col)
    if (!is.null(dist_txt)) {
  ...
  lines <- c(lines,
    sep_thick,
    sprintf("  End of maternal subsample report -- %s", timestamp),
    sep_thick
  )
...
# ==============================================================================
# MAIN FUNCTION: run_subsample_maker_maternal
# ==============================================================================

run_subsample_maker_maternal <- function(df, cfg) {

  DATE_COL <- "dateadmission"
  ...
  if (!DATE_COL %in% names(df)) {
    stop(sprintf(
      "[maternal] Date column '%s' not found in input file.\n",
      DATE_COL
    ))
  }
  ...
  df$date_parsed <- .mat_parse_date(df[[DATE_COL]])
  ...
  filter_info <- .mat_build_filter_info(cfg)
  cat(sprintf("  Filter: %s\n", filter_info$desc))

  # Apply date/facility filter
  n_before      <- nrow(df)
  df_filtered   <- .mat_apply_filter(df, cfg)
  ...
  has_excl <- !is.null(cfg$sub_exclusion_filters) &&
              length(cfg$sub_exclusion_filters) > 0
  if (has_excl) {
    df_filtered <- .mat_apply_exclusion_filters(
  ...
  df_out <- .mat_select_columns(df_filtered, cfg$sub_variables)

  # Build report (while date_parsed is still present for range stats)
  report <- .mat_build_report(
    df_in      = df,
    df_out     = df_out,
...
# ==============================================================================
# WRITE OUTPUTS: write_subsample_outputs_maternal
# ==============================================================================

write_subsample_outputs_maternal <- function(result, prefix, out_dir,
                                             stem = "subsample_maternal") {
