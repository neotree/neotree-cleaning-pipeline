...
|---------|------|---------|
| v8 | 2026-08-09 | **Logging fix — Module 15 was silently aborting for two MWI maternity datasets.** `derive_maternal_age_columns()` built its disagreement warning with its own `sprintf()` and handed the finished string to `log_warn()`, but `00_setup.r` sets `log_formatter(formatter_sprintf)`, so the logger formatted it a second time: the first pass renders `%.1f%%` as a literal percent sign (e.g. `(12.5%);`), the second hits `%)` and R throws `Error: too few arguments`. **`mwi_*_combined_maternity_outcomes` and `mwi_*_dhis2_maternal_outcomes` therefore produced no `*_cleaned.csv`, no `*_cleaned.rds` and no `15b_maternal_age_summary.txt` on every previous run** — a data-availability failure, not a correctness one (no value was ever computed wrongly). Only these two datasets were affected: the failing branch needs both maternal-age sources non-NA on the same row and disagreeing by >1 year, and `matagedate` is MWI-only and only on the maternity forms. Fixed by passing the format string and its arguments to the logger instead of pre-formatting. **The same construction was normalised at all 21 `log_*(sprintf(...))` sites pipeline-wide**, two of which were live rather than latent hazards because they interpolate rejected raw data values into the message (`12_boolean_validation.r:70`, `14_datetime_validation.r:105` — any `%` in a source cell would have aborted those modules identically). Rule going forward: never hand a pre-formatted string to `log_info`/`log_warn`/`log_error`/`log_debug`; pass the format string plus arguments and let the logger format once. **`run_all.r`** now captures `conditionMessage(e)` per file and prints a failure summary (file, dataset, error) at the end — the missing piece that let this go unnoticed. **Verified:** a full batch run completed for every input file, each reaching `Module 15 complete`. See `how_to_run.md` → "Failure summary". |
| v8 | 2026-07-10 | **Harmonised maternal-age variable implemented (Modules 11 + 15).** Maternal age is captured on the deliveries form in two fields that no single column cleans: `matageyrs` (manual whole years, both sites, range 9–60) and `matagedate` (auto-calculated from DOB, MWI only, **stored in hours** — divide by 8766 = 365.25×24 for years). **Module 11** now derives `mat_age_date_years = round(matagedate / 8766)` and applies the same 9–60 plausibility filter (out-of-range → NA, counted in the Module 11 report), leaving raw `matagedate` untouched; the existing `matageyrs` hours-rescue divisor was standardised 8760 → **8766** to match. **Module 15** `derive_maternal_age_columns()` (after suffix stripping, before output, mirroring `derive_weight_columns()`) adds **`mat_age_years_combined = coalesce(matageyrs, mat_age_date_years)`** (manual field wins) and provenance **`mat_age_source ∈ {matageyrs, matagedate_derived, none}`**, emitted only where a source column is present so ZIM (matageyrs only) and MWI (both) share one schema. **Disagreement guard:** rows where both fields are present and differ by >1 year are counted and logged (never overwritten) → `15b_maternal_age_summary.txt`. Implausible ages flow through the standard machinery: filtered in Module 11, NA-reason-coded **-8 (invalid/removed)** by Module 16 on the source columns. **Verified** on a full extract for both countries: the coalesce, the provenance labelling and the disagreement counter all behave as specified, and rows where the two sources disagree are flagged rather than overwritten. Note that maternal age was added to the deliveries form partway through deployment, so baseline missingness on that form is a collection-timing artefact rather than data loss — check the field’s start date for your site before interpreting completeness. **NB (superseded 2026-08-09):** this feature was originally verified by logic checks + a Python dry-run only, never end-to-end in R — which is how the logging defect in the disagreement-warning branch (see the 2026-08-09 entry) shipped undetected and suppressed all output for the two MWI maternity datasets. It has since been run end-to-end in R and verified. See `readme.md` → "Maternal age". |
